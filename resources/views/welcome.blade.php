...
<head>
    ...
    <title>SIMBA PTKI - Pendaftaran KIP Kuliah</title>
...
<body class="bg-slate-50 text-slate-800 font-sans antialiased selection:bg-indigo-500 selection:text-white">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-16">
                <div class="flex-shrink-0 flex items-center gap-2">
                    <div
                        class="w-8 h-8 bg-indigo-600 text-white rounded-lg flex items-center justify-center font-bold text-xl">
                        S</div>
                    <span class="text-xl font-bold text-slate-900 tracking-tight">SIMBA</span>
                </div>

                <div class="hidden md:flex items-center space-x-8">
                    <a href="#syarat-mutlak"
                        class="text-sm font-medium text-slate-600 hover:text-indigo-600 transition">Syarat Wajib</a>
                    <a href="#kriteria-saw"
                        class="text-sm font-medium text-slate-600 hover:text-indigo-600 transition">Kriteria
                        Penilaian</a>
                    <a href="#berkas"
                        class="text-sm font-medium text-slate-600 hover:text-indigo-600 transition">Berkas</a>
                </div>

                <div class="hidden md:flex items-center space-x-4">
                    @auth
                        <a href="{{ url('/dashboard') }}"
                            class="text-sm font-semibold text-slate-700 hover:text-indigo-600">Dasbor Saya</a>
                    @else
                        <a href="{{ route('login') }}"
                            class="text-sm font-semibold text-slate-600 hover:text-indigo-600">Masuk</a>
                        <a href="{{ route('register') }}"
                            class="text-sm font-semibold bg-indigo-600 text-white px-5 py-2 rounded-full shadow-sm hover:bg-indigo-700 hover:shadow transition">Daftar
                            Beasiswa</a>
                    @endauth
                </div>
        ...
        <div x-cloak x-show="open" class="md:hidden bg-white border-t border-slate-100 shadow-lg absolute w-full">
            <div class="px-4 pt-2 pb-4 space-y-1">
                <a href="#syarat-mutlak" @click="open = false"
                    class="block px-3 py-2 rounded-md font-medium text-slate-700 hover:text-indigo-600 hover:bg-slate-50">Syarat
                    Wajib</a>
                <a href="#kriteria-saw" @click="open = false"
                    class="block px-3 py-2 rounded-md font-medium text-slate-700 hover:text-indigo-600 hover:bg-slate-50">Kriteria
                    Penilaian</a>
                <a href="#berkas" @click="open = false"
                    class="block px-3 py-2 rounded-md font-medium text-slate-700 hover:text-indigo-600 hover:bg-slate-50">Berkas</a>
                <div class="border-t border-slate-200 pt-4 mt-2">
                    @auth
                        <a href="{{ url('/dashboard') }}"
                            class="block px-3 py-2 font-semibold text-indigo-600 hover:bg-slate-50 rounded-md">Dasbor
                            Saya</a>
                    @else
                        <a href="{{ route('login') }}"
                            class="block px-3 py-2 font-medium text-slate-700 hover:bg-slate-50 rounded-md">Masuk</a>
                        <a href="{{ route('register') }}"
                            class="block px-3 py-2 mt-2 text-center font-semibold bg-indigo-600 text-white rounded-md hover:bg-indigo-700">Daftar
                            KIP-K</a>
                    @endauth
                </div>
            </div>
    ...
    {{-- <section class="relative bg-white pt-20 pb-28 border-b border-slate-200">
        <div class="max-w-5xl mx-auto px-4 text-center">
            <span class="inline-block py-1 px-3 rounded-full bg-indigo-50 text-indigo-600 text-sm font-semibold mb-6 border border-indigo-100">Evaluasi KIP Kuliah 2025</span>
            <h1 class="text-4xl md:text-5xl font-extrabold text-slate-900 tracking-tight mb-6 leading-tight">
                Transparansi Seleksi Melalui <br/><span class="text-indigo-600">Sistem Pendukung Keputusan</span>
            </h1>
            ...
            <div class="flex flex-col sm:flex-row justify-center gap-4">
                <a href="{{ route('register') }}" class="bg-indigo-600 text-white px-8 py-3 rounded-full font-semibold text-lg hover:bg-indigo-700 shadow-md transition">Daftar Sekarang</a>
    ...
    <section class="relative bg-white pt-24 pb-32 overflow-hidden">
        ...
        <div class="relative max-w-5xl mx-auto px-4 text-center">
            ...
            <div class="flex flex-col sm:flex-row justify-center gap-4">
                <a href="{{ route('register') }}"
                    class="bg-indigo-600 text-white px-8 py-3.5 rounded-full font-semibold text-lg hover:bg-indigo-700 shadow-lg hover:shadow-indigo-500/30 transition transform hover:-translate-y-0.5">Mulai
                    Pendaftaran</a>
                <a href="#syarat-mutlak"
        ...
        <div class="max-w-7xl mx-auto px-4">
            <div class="flex flex-col lg:flex-row gap-12 items-start">
                ...
                <div class="w-full lg:w-2/3 grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="p-4 bg-indigo-50 rounded-lg border border-indigo-100 shadow-sm">
                        <div class="flex justify-between items-center mb-2">
                            <h4 class="font-bold text-indigo-900">Potensi Akademik (C1)</h4>
                            <span class="text-xs bg-green-200 text-green-800 px-2 py-1 rounded font-bold">Benefit •
                                20%</span>
                        </div>
                        <p class="text-sm text-indigo-700">Dilihat dari standar Angka Rata-rata Nilai Rapor atau IPK
                            Awal.</p>
                    </div>

                    <div class="p-4 bg-indigo-50 rounded-lg border border-indigo-100 shadow-sm">
                        <div class="flex justify-between items-center mb-2">
                            <h4 class="font-bold text-indigo-900">Prestasi Non-Akademik (C2)</h4>
                            <span class="text-xs bg-green-200 text-green-800 px-2 py-1 rounded font-bold">Benefit •
                                10%</span>
                        </div>
                        <p class="text-sm text-indigo-700">Penilaian berdasarkan tingkat kejuaraan yang diraih.</p>
                    </div>

                    <div class="p-4 bg-indigo-50 rounded-lg border border-indigo-100 shadow-sm">
                        <div class="flex justify-between items-center mb-2">
                            <h4 class="font-bold text-indigo-900">Penghasilan Ortu (C3)</h4>
                            <span class="text-xs bg-red-200 text-red-800 px-2 py-1 rounded font-bold">Cost • 25%</span>
                        </div>
                        <p class="text-sm text-indigo-700">Semakin kecil pendapatan, bobot skor sistem semakin tinggi.
                        </p>
                    </div>

                    <div class="p-4 bg-indigo-50 rounded-lg border border-indigo-100 shadow-sm">
                        <div class="flex justify-between items-center mb-2">
                            <h4 class="font-bold text-indigo-900">Status Kesejahteraan (C4)</h4>
                            <span class="text-xs bg-green-200 text-green-800 px-2 py-1 rounded font-bold">Benefit •
                                15%</span>
                        </div>
                        <p class="text-sm text-indigo-700">Pemilik KIP, KKS, atau KJP menjadi skala prioritas
                            tertinggi.</p>
                    </div>

                    <div class="p-4 bg-indigo-50 rounded-lg border border-indigo-100 shadow-sm">
                        <div class="flex justify-between items-center mb-2">
                            <h4 class="font-bold text-indigo-900">Kondisi Khusus (C5)</h4>
                            <span class="text-xs bg-green-200 text-green-800 px-2 py-1 rounded font-bold">Benefit •
                                10%</span>
                        </div>
                        <p class="text-sm text-indigo-700">Prioritas tambahan bagi mahasiswa Yatim/Piatu.</p>
                    </div>

                    <div class="p-4 bg-indigo-50 rounded-lg border border-indigo-100 shadow-sm">
                        <div class="flex justify-between items-center mb-2">
                            <h4 class="font-bold text-indigo-900">Jumlah Tanggungan (C6)</h4>
                            <span class="text-xs bg-green-200 text-green-800 px-2 py-1 rounded font-bold">Benefit •
                                10%</span>
                        </div>
                        <p class="text-sm text-indigo-700">Makin banyak anggota keluarga ditanggung, skor makin besar.
                        </p>
                    </div>

                    <div class="p-4 bg-indigo-50 rounded-lg border border-indigo-100 shadow-sm sm:col-span-2">
                        <div class="flex justify-between items-center mb-2">
                            <h4 class="font-bold text-indigo-900">Nilai Kepesantrenan (C7)</h4>
                            <span class="text-xs bg-green-200 text-green-800 px-2 py-1 rounded font-bold">Benefit •
                                10%</span>
                        </div>
                        <p class="text-sm text-indigo-700">Kesiapan pembinaan asrama pesantren sesuai ciri khas PTP
                            Kemenag.</p>
                    </div>
                </div>
        ...
        <div class="max-w-6xl mx-auto px-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div class="bg-slate-800 p-6 rounded-xl border border-slate-700">
                    <h3 class="text-lg font-bold text-indigo-400 mb-4 border-b border-slate-700 pb-2">Berkas Utama
                        (Wajib)</h3>
                    <ul class="space-y-3">
                        <li class="flex items-start">
                            <span class="text-green-400 mr-2 mt-0.5">✔</span>
                            <span class="text-sm text-slate-300">KTP & Kartu Keluarga (KK).</span>
                        </li>
                        <li class="flex items-start">
                            <span class="text-green-400 mr-2 mt-0.5">✔</span>
                            <span class="text-sm text-slate-300"><strong>Ijazah & Transkrip Nilai Terakhir</strong>
                                (Dilegalisir, sebagai bukti Rata-rata Nilai untuk kriteria C1).</span>
                        </li>
                        <li class="flex items-start">
                            <span class="text-green-400 mr-2 mt-0.5">✔</span>
